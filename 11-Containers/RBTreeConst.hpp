...
namespace ft {
	...
	class RBTreeConst {
		public:
        	...
        	typedef ft::treeRBIterator< const T >								iterator;
        	...
        	typedef node< const value_type >                                          treeNode;
		...
		public:
			explicit RBTreeConst(const value_compare &comp, const allocator_type &alloc) : _NIL(nilNode()), _height(0), _comp(comp), _alloc(alloc) {
				_NIL->color = BLACK;
				_root = _NIL;
			}
			
 			RBTreeConst( RBTreeConst const &t ) : _NIL(nilNode()), _root(_NIL), _comp(t._comp), _alloc(t._alloc) { insert(t.begin(), t.end()); };

			RBTreeConst& operator=(const RBTreeConst &t) {
				if (this != &t) {
					clear(_root);
		...
		private:
        	...
        	treeNode	*newNode( value_type const& value, treeNode *parent, int leaf ) {
        	    ...
        	    _alloc.construct(const_cast< T *>(&(newnode->value)), value);
        	    newnode->left  = _NIL;
        	    newnode->right = _NIL;
        	...
        	void	delNode(treeNode *node) {
        	    _alloc.destroy( const_cast< T *>(&(node->value)));  
        	    _node_alloc.deallocate(node, 1); 
        	    _height--;
        	}
